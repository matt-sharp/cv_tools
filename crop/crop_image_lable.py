...
class SplitBase():
    def __init__(self,
        ...
        self.img_path = self.basepath / f"VOC{year}/JPEGImages"
        self.anno_path = self.basepath / f"VOC{year}/Annotations"
        self.train_sets = open(
            self.basepath / f'VOC{year}/ImageSets/Main/train.txt').read().strip().splitlines()
        self.val_sets = open(
            self.basepath / f'VOC{year}/ImageSets/Main/val.txt').read().strip().splitlines()
    ...
    def mk_voc_dir(self, root, year=2019):
        if isinstance(root, str):
            root = Path(root)
        anno = root / f"VOC{year}/Annotations"
        img = root / f"VOC{year}/JPEGImages"
        sets = root / f"VOC{year}/ImageSets/Main"
...
def main():
    ...
    # parser.add_argument('-i', '--inp_voc_root', type=str,default='/data1/',
    #                     help='orginal VOC dataset root directory')
    # parser.add_argument('-o', '--output', type=str,default='/data1/temp',
